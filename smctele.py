...
def handle_command(text: str) -> None:
    ...
    with state_lock:
        ...
        state["last_command_time_utc"] = now_utc_str()
    ...
    if command == "/wakehelp":
        send_telegram_message(
            "ℹ️ *Render free wake-up help*\n\n"
            "If commands stop responding after some idle time:\n"
            "1. Open your Render URL or `/wake`\n"
                ...
                if AUTO_CAPTURE_CHAT_ID and not get_effective_chat_id() and chat_id:
                    ...
                    if SEND_RESTART_MESSAGE:
                        send_telegram_message(
                            ...
                            "On Render free, if commands stop after idle time, open the service URL or `/wake`, wait 30-60 seconds, then send the command again."
                        )
...
if SEND_RESTART_MESSAGE and TELEGRAM_CHAT_ID:
    send_telegram_message(
        "✅ XAUUSD SMC bot restarted on Render and background loops started.\n\n"
        "On Render free, if commands stop after idle time, open the service URL or `/wake`, wait 30-60 seconds, then send the command again."
    )
